    ...
    <div className="space-y-6">
              ...
              {filteredOrders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="border-b border-white/[0.04] transition-colors hover:bg-white/[0.04]">
                    ...
                    <td className="py-3 pr-4 text-zinc-400">{o.seller}</td>
      ...
      <div className="rounded-[16px] bg-[#111] p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-white">Open Disputes ({disputes.filter((d) => d.status !== 'resolved').length})</h2>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-zinc-500">Order {d.orderId}</span>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${disputeStatusColors[d.status]}`}>
                      {d.status}
                    </span>
                  </div>
                  <p className="mt-1 text-sm font-medium text-white">{d.reason}</p>
          ...
          <div className="w-full max-w-lg rounded-[16px] bg-[#111] p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-[#0a0a0a] p-4">
                ...
                <p className="text-base font-medium text-white">{selectedOrder.product}</p>
                <p className="mt-1 text-2xl font-bold text-[#FF6B35]">${selectedOrder.total.toFixed(2)}</p>
                <p className="text-xs text-zinc-500">Platform fee: ${selectedOrder.fee.toFixed(2)}</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-white/[0.06] p-3">
                  <p className="text-xs text-zinc-500">Buyer</p>
                  <p className="text-sm font-medium text-white">{selectedOrder.buyer}</p>
                  <p className="text-xs text-zinc-500">{selectedOrder.buyerEmail}</p>
                </div>
                <div className="rounded-xl border border-white/[0.06] p-3">
                  <p className="text-xs text-zinc-500">Seller</p>
                  <p className="text-sm font-medium text-white">{selectedOrder.seller}</p>
                  <p className="text-xs text-zinc-500">{selectedOrder.sellerEmail}</p>
                </div>
              </div>
              <div className="rounded-xl border border-white/[0.06] p-3">
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button className="flex-1 rounded-xl bg-white/[0.06] py-2.5 text-sm font-medium text-zinc-300 hover:bg-white/[0.1]">
                Contact Parties
              </button>
            </div>
          </div>
