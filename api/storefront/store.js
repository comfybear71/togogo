...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (productId && /^[0-9a-f-]{36}$/i.test(productId)) {
      ...
      // to mention "moments", "elements", "agreement", "improvements",
      // etc. (because "men" is a substring of all of them) — which is
      // exactly why customers see a near-random handful of unrelated
      // products and the same shuffle every time.
      //
      ...
      if (search) {
        ...
        for (const w of words) {
          ...
          // NOT "moments" or "elements".
          whereExtra += ` AND ${haystack} LIKE '% ${w}%'`
        }
        // If every word was noise (e.g. user typed "of in"), fall
        // back to a basic word-boundary match on the original input.
        if (words.length === 0) {
